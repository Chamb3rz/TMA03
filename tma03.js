...
function addTextEntry(itemKey, initialText, isNewEntry) {
    ...
    function saveEntry() {
        ...
        // Q1(c)(iii) Task 1 of 2
        // ...get the textarea element's current value
        initialText = textElement.value;

        // ...make a text item using the value
        var diaryItem = makeDiaryItem("text", initialText);

        // ...store the item in local storage using the given key
        localStorage.setItem(itemKey, diaryItem);

    }

    // Q1(c)(iii) Task 2 of 2
...
function processFile(changeEvent) {
    ...
    function addImage(dataUrl) {
        ...
        var diaryItem = makeDiaryItem("image", dataUrl);
    ...
    // Get a reference to the file input, the target of the change event
    var inputElement = changeEvent.target;

    // Get the first selected file from the file input's files array
    // (if this is a multiple file input there will be more items in this array)
    var file = inputElement.files[0];

    function loaded(event) {
        // Get a "data URL" (a long string of text encoding the binary data)
        // from the load event target...
    ...
    // Attach a listener that will add our image
...
function initialize() {
    ...
    if (!window.localStorage) {
        ...
        // https://developer.mozilla.org/en-US/docs/Web/    /Web_Storage_API/Using_the_Web_Storage_API#Testing_for_availability
